    ...
    if (token) {
        if (window.adminRef) {
            adminRef.get().then(function (docSnap) {
                ...
                var storedVersion = sessionStorage.getItem('makka_admin_version');
    ...
    adminRef.get().then(function (docSnap) {
        ...
        hashString(pass).then(function (passHash) {
            if (user === creds.username && passHash === creds.passwordHash) {
                ...
                var dbVersion = String(creds.sessionVersion || '1');
                sessionStorage.setItem('makka_admin', sessionToken);
                sessionStorage.setItem('makka_admin_version', dbVersion);
                document.getElementById('loginScreen').style.display = 'none';
                document.getElementById('adminPanel').style.display = 'block';
                initializeAdmin();
            } else {
